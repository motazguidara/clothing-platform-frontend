...
export default function FilterDrawer() {
  ...
  return (
    <Sheet open={isOpen} onOpenChange={(v) => (v ? null : close())}>
      <SheetContent side="right" className="w-[360px] bg-white text-black border-border z-[60]">
        <SheetHeader>
          <SheetTitle>Filters</SheetTitle>
        </SheetHeader>
        <div className="py-5 px-3 sm:px-4 space-y-7">
          <div>
            <div className="text-xs font-semibold uppercase tracking-tight">Category</div>
          ...
          <div>
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {["#111","#f00","#0f0","#00f","#aaa"].map((hex) => (
                <button
                  key={hex}
                  className={`h-6 w-6 rounded-full border ${filters.colors.includes(hex) ? 'ring-2 ring-offset-2 ring-black' : ''}`}
          ...
          <div className="pt-2 grid grid-cols-2 gap-2">
            <Button variant="secondary" onClick={() => filters.clear()}>Clear</Button>
