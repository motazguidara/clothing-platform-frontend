...
export default function SearchPage() {
  ...
  const allow = ["q", "category", "gender", "price_min", "price_max", "size", "color", "ordering", "page", "sale", "in_stock"];
    ...
    <section className="max-w-7xl mx-auto px-6 pb-16 pt-36">
      ...
      <div className="mb-6 flex flex-wrap items-center gap-2">
        {Object.entries(params).map(([key, value]) => {
          if (key === "page" || key === "ordering" || !value) return null;
          return (
            <div key={key} className="flex items-center gap-1 px-2 py-1 bg-gray-100 rounded-full text-xs">
        ...
        <div className="mt-10 flex items-center justify-center gap-2">
          {(() => {
            ...
            return (
              <>
                <button
                  onClick={() => current > 1 && router.push(makeHref(current - 1))}
                  disabled={current <= 1}
                ...
                <button
                  onClick={() => current < total && router.push(makeHref(current + 1))}
                  disabled={current >= total}
                  className={`px-3 py-2 border rounded-md text-sm ${
                    current >= total ? 'opacity-50 cursor-not-allowed' : 'hover:bg-gray-100'
                  }`}
                >
                  Next
                </button>
      ...
      {!currentQuery && !isLoading && (
        <div className="mt-16 border-t pt-8">
          ...
          <div className="flex flex-wrap gap-2">
            {["dress", "jeans", "sneakers", "hoodie", "t-shirt", "jacket"].map((term) => (
              <button
                key={term}
                onClick={() => handleSearch(term)}
                className="px-3 py-1 bg-gray-100 rounded-full text-sm hover:bg-gray-200 transition-colors"
